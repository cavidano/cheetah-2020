...
use Exception;
use \threewp_broadcast\premium_pack\classes;
...
{
	use classes\blogs_trait;
	use classes\files_trait;
	use classes\find_unlinked_children_on_blog;
	...
	**/
	...
	/**
		@brief		Which site are we copying?
		@details	Used to help the copy_recursive_this_directory decide whether to copy specific directories.
		@since		2021-01-16 20:52:50
	**/
	public static $copying_site = false;
	...
	**/
	public function broadcast_bulk_cloner_clone_blog( $action )
	{
		...
		$requested_blog_id = $bs->get_blog_id();
		...
		if ( $requested_blog_id > 0 )
			if ( $requested_blog_id != $blog_id )
			{
				try
				{
					$this->change_blog_id( $blog_id, $requested_blog_id );
					$blog_id = $requested_blog_id;
				}
				catch ( Exception $e )
				{
					$this->debug( 'Unable to change blog ID from %s to requested %s: %s', $blog_id, $requested_blog_id, $e->getMessage() );
				}
			}

		// This is to prevent warnings later during a collection->has() call during find_unlinked_children_on_blog.php:93
		$blog_id = intval( $blog_id );

		// Blog has now been created!
		$bs->collection( 'blog' )->set( 'blog_id', $blog_id );

		$this->debug( 'Created blog %d (%s) from blog %d. Now cloning tables.', $bs->get_blog_id(), $new_site_domain_data->url, $action->template_blog_id );
		...
		foreach( $rows as $row )
		{
			...
			if ( is_array( $data ) )
			{
				...
				$data = static::replace_string_in_array( $data, $old_site_url, $new_site_domain_data->url );
				$update = true;
			}
		...
		if ( ! $copy_action->is_finished() )
		{
			...
			self::$copying_site = $action->template_blog_id;
	...
	**/
	...
	{
		if ( self::$copying_site > 1 )
			return true;
		// Do not copy the sites directory.
		return strpos( $directory, 'sites' ) !== 0;
	}
	...
	/**
		...
		@since		2021-01-16 20:14:33
	**/
	public static function copy_recursive_this_file( $file )
	{
		return static::copy_recursive_this_directory( $file );
	}
	...
	**/
	public static function replace_string_in_array( $array, $replace_this, $with_this )
	{
