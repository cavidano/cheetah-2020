	...
	**/
	public function setup()
	{
		...
		if ( $form->is_posting() )
		{
			...
			// Readd the slashes.
			foreach( [ 'setup_code', 'loop_code', 'teardown_code' ] as $type )
				$form->input( $type )->value( stripslashes( $form->input( $type )->get_post_value() ) );
			...
			if ( $run_button->pressed() )
			{
				try
				{
					...
					$action->code->setup = stripslashes( $form->input( 'setup_code' )->get_post_value() );
					$action->code->loop = stripslashes( $form->input( 'loop_code' )->get_post_value() );
					$action->code->teardown = stripslashes( $form->input( 'teardown_code' )->get_post_value() );
