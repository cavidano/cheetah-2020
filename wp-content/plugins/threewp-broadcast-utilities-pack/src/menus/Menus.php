	...
	extends \threewp_broadcast\premium_pack\base
{
	...
	**/
	...
	public function _construct()
	{
		...
		// This is for the WP Menu Icons plugin.
		add_action( 'broadcast_menus_copy_menus', function()
		{
			if ( class_exists( 'WPMI_Backend' ) )
			{
				$wpmi = \WPMI_Backend::instance();
				remove_action('wp_update_nav_menu_item', [ $wpmi, 'wp_update_nav_menu_item' ], 10, 3 );
			}
		}, 5 );
	}
	...
	public function admin_manipulate_item()
	{
		...
		else
		{
			...
			foreach( $menu_items as $item )
			{
				...
				if ( $item->type == 'custom' )
					$title .= ' (' . $item->url . ')';
				$menu_items_select->option( $title, $item->ID );
			}
			...
			if ( $form->is_posting() )
			{
				...
				$action = $this->new_action( 'manipulate_menu_item_setup' );
				$action->data = $data;
				$action->execute();
				...
				foreach( $data->blogs as $blog_id )
				{
					...
					foreach( $blog_menus as $blog_menu )
					{
						if ( $blog_menu->name == $data->menu->name )
						{
							...
							if ( ( strpos( $data->action, 'delete' ) !== false ) )
							{
								// Have we not found the equivalent? Perhaps it's a custom?
								if ( ! $equivalent_menu_item_id )
								{
									// Try to find the equivalent using the post title and URL.
									if ( $data->menu_item->type == 'custom' )
									{
										// The type, title and url must match.
										foreach( $blog_menu_items as $menu_item_id => $menu_item )
											if ( $menu_item->type == 'custom' )
												if ( $menu_item->title == $data->menu_item->title )
													if ( $menu_item->url == $data->menu_item->url )
														$equivalent_menu_item_id = $menu_item->ID;
									}
								}

								if ( $equivalent_menu_item_id > 0 )
								{
							...
							if ( strpos( $data->action, 'create' ) !== false )
							{
								...
								if ( $data->menu_item->type == 'custom' )
									$new_item[ 'menu-item-url' ] = $data->menu_item->url;
								...
								{
									$this->debug( 'Menu item has a parent' );
									if ( isset( $data->menu_item_parent->broadcast_data ) )
									{
									...
									{
										// Try to find the menu item with the same post name.
										foreach( $blog_menu_items as $blog_menu_item )
											if ( $blog_menu_item->title == $data->menu_item_parent->title )
											{
												$this->debug( 'Equivalent menu item parent: %d', $blog_menu_item->ID );
												$new_item[ 'menu-item-parent-id' ] = $blog_menu_item->ID;
											}
	...
	**/
	public function admin_menu_copy()
	{
		...
		$replace_url_input = $form->checkbox( 'replace_url' )
			...
			->description( sprintf(
				// Input title. Options for broadcasting menus.
				__( "Replace the parent domain's URL with that of the child. %s will become %s", 'threewp_broadcast' ),
	...
	public function broadcast_menus_copy_menu( $action )
	{
		...
		foreach( $action->menu as $index => $menu_item )
		{
			...
			foreach( $action->menu as $parent_item )
				...
				{
					$parent = $parent_item;
					$this->debug( 'Parent for %s found: %s', $menu_item->ID, $parent );
					break;
				}
			if ( $parent === null )
...
/**
								...
								if ( $data->menu_item_parent )
								{
									...
									else
									{
										// Try to find the menu item with the same post name.
										foreach( $blog_menu_items as $blog_menu_item )
											if ( $blog_menu_item->post_name == $data->menu_item_parent->post_name )
											{
												$this->debug( 'Equivalent menu item parent: %d', $blog_menu_item->ID );
												$new_item[ 'menu-item-parent-id' ] = $blog_menu_item->ID;
											}
...
**/
	...
	**/
	public function broadcast_menus_copy_menu_item( $action )
	{
		...
		$this->debug( 'Ran wp_update_nav_menu_item (%s) with %s, 0, %s',
			$menu_item->new_item_id,
			$action->menu_id,
			$modify_new_menu_item->new_item
		);
