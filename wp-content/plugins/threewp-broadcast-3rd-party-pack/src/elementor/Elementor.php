		...
		if ( isset( $element->settings ) )
		{
			...
			if ( isset( $element->settings->background_image ) )
				if ( $element->settings->background_image->id > 0 )
				{
					...
					{
						$this->debug( 'Warning! Broken background image %s', $image_id );
					}
				}
		}
		...
		if ( $element->elType == 'widget' )
		{
			switch( $element->widgetType )
			{
				...
				case 'image-box':
					...
					{
						$this->debug( 'Warning! Broken image widget %s', $image_id );
					}
					break;
				case 'image-gallery':
					foreach( $element->settings->wp_gallery as $gallery_index => $gallery_item )
					{
						...
						{
							$this->debug( 'Warning! Broken gallery widget image %s', $image_id );
						}
					}
					break;
				case 'smartslider':
