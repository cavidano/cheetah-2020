...
class DUPX_DBInstall
{
    ...
    protected function constructData()
    {
        ...
        $this->post = array(
            'view_mode'         => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_VIEW_MODE),
            'dbaction'          => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_ACTION),
            ...
            'dbport'            => parse_url($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_HOST), PHP_URL_PORT),
            'dbchunk'           => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_CHUNK),
            'dbnbsp'            => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_SPACING),
            'dbmysqlmode'       => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_MYSQL_MODE),
            'dbmysqlmode_opts'  => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_MYSQL_MODE_OPTS),
            'dbobj_views'       => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_VIEW_CREATION),
            'dbobj_procs'       => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_PROC_CREATION),
            'dbobj_funcs'       => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_FUNC_CREATION),
            'dbsplit_creates'   => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_SPLIT_CREATES),
            'dbcharset'         => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_CHARSET),
            'dbcollate'         => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_COLLATE),
            'cpnl-host'         => $paramsManager->getValue(DUPX_Params_Manager::PARAM_CPNL_HOST),
            'cpnl-user'         => $paramsManager->getValue(DUPX_Params_Manager::PARAM_CPNL_USER),
            'cpnl-pass'         => $paramsManager->getValue(DUPX_Params_Manager::PARAM_CPNL_PASS),
            'cpnl-dbuser-chk'   => $paramsManager->getValue(DUPX_Params_Manager::PARAM_CPNL_DB_USER_CHK),
            'pos'               => 0,
            'pass'              => false,
            'first_chunk'       => true,
            'dbchunk_retry'     => 0,
            'continue_chunking' => $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_CHUNK),
            'progress'          => 0,
            'delimiter'         => ';',
            'is_error'          => 0,
            'error_msg'         => ''
        ...
        $this->dbaction         = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_ACTION);
        $this->dbcharset        = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_CHARSET);
        $this->dbcollate        = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_COLLATE);
        $this->dbobj_views      = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_VIEW_CREATION);
        $this->dbobj_procs      = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_PROC_CREATION);
        $this->dbobj_funcs      = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_FUNC_CREATION);
        $this->dbsplit_creates  = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_SPLIT_CREATES);
        $this->dbchunk          = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_CHUNK);
    ...
    protected function initLogDbInstall()
    {
        ...
        DUPX_Log::info(str_pad('DB ENGINE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_ENGINE)));
        DUPX_Log::info(str_pad('VIEW MODE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_VIEW_MODE)));
        DUPX_Log::info(str_pad('DB ACTION', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_ACTION)));
        ...
        DUPX_Log::info(str_pad('KEEP USERS', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_KEEP_TARGET_SITE_USERS)));
        DUPX_Log::info(str_pad('CONTENT OWNER', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_CONTENT_OWNER)));
        DUPX_Log::info(str_pad('TABLE PREFIX', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX)));
        DUPX_Log::info(str_pad('MYSQL MODE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_MYSQL_MODE)));
        DUPX_Log::info(str_pad('MYSQL MODE OPTS', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_MYSQL_MODE_OPTS)));
        DUPX_Log::info(str_pad('NON-BREAKING SPACES', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_SPACING)));
        DUPX_Log::info(str_pad('CHARSET', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_CHARSET)));
        DUPX_Log::info(str_pad('COLLATE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_COLLATE)));
        DUPX_Log::info(str_pad('CUNKING', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_CHUNK)));
        DUPX_Log::info(str_pad('VIEW CREATION', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_VIEW_CREATION)));
        DUPX_Log::info(str_pad('STORED PROCEDURE', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_PROC_CREATION)));
        DUPX_Log::info(str_pad('FUNCTIONS', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_FUNC_CREATION)));
        DUPX_Log::info(str_pad('SPLIT CREATES', $labelPadSize, '_', STR_PAD_RIGHT) . ': ' . DUPX_Log::varToString($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_SPLIT_CREATES)));
    ...
    public function deploy()
    {
        ...
        switch ($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_ACTION)) {
    ...
    {
        $paramsManager = DUPX_Params_Manager::getInstance();
        if ($paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_CHUNK)) {
    ...
    protected function moveUserTableOnCurrent()
    {
        ...
        if ($overwriteData['table_prefix'] != $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX)) {
            ...
            DUPX_Log::info('RENAME USERS TABLE FROM ' . $targetUserTable . ' TO ' . $currentUserTableName);
            $dbFunc->renameTable($targetUserTable, $currentUserTableName, true);
            DUPX_Log::info('RENAME USERS META TABLE FROM ' . $targetUserMetaTable . ' TO ' . $currentUserMetaTableName);
            ...
            DUPX_UpdateEngine::updateTablePrefix($dbFunc->dbConnection(),
                                                 ...
                                                 $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX)
            );
        } else {
            DUPX_Log::info('TABLE NAMES ARE THE SAME, SO SKIP USERS TABLES RENAME');
        }

        $this->updatPostsTableAuthorOnKeepUser();

        DUPX_Log::info("SITE USERS UPDATED\n");
    }

    protected function updatPostsTableAuthorOnKeepUser()
    {
        if (($keepUser = DUPX_Params_Descriptor_users::getKeepUserId()) == 0) {
            return;
        }

        DUPX_Log::info('UPDATE POSTS AUTHOR ID ' . $keepUser);
        ...
        if (is_null($skipRegex)) {
            ...
            if (count($skipTables) === 0) {
                ...
                if (DUPX_ArchiveConfig::getInstance()->dbInfo->buildMode === self::BUILD_MODE_MYSQLDUMP) {
                    $skipRegex = '/^\s*(?:\/\*!\d+\s)?\s*(?:CREATE|INSERT|ALTER|LOCK)\s.*(?:TABLE|INTO).*[`\s](' . implode('|', $skipTables) . ')[`\s]/im';
                } else {
        ...
        if (is_null($sreachRegEx)) {
            ...
            foreach ($mapping as $oldPrefix => $newMapping) {
                ...
                foreach ($newMapping as $newPrefix => $commons) {
                    DUPX_LOG::info('CANGHE TABLES PREFIX ' . $oldPrefix . ' TO ' . $newPrefix . 'FOR ' . DUPX_Log::varToString($commons));
                    ...
                    $sreachRegEx[]  = '/' . $quoteOldPrefix . '(' . implode('|', $quoteCommons) . ')/m';
                    $replaceRegEx[] = $quoteNewPrefix . '$1';
                    $sreachRegEx[]  = '/(CONSTRAINT[\s\t]+[`\'"]?.+)(?-i)' . $quoteOldPrefix . '(?i)(.+[`\'"]?[\s\t]+FOREIGN[\s\t]+KEY)/mi';
    ...
    public function runCleanupRoutines()
    {
        if (DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_RESTORE_BACKUP_MODE)) {
            ...
            DUPX_Log::info("CLEANUP ROUTINE ADD ON MULTISITE START");
        } else {
    ...
    private function emptyDuplicatorPackages()
    {
        ...
        DUPX_DB::mysqli_query($this->dbh, "DELETE FROM `" . $packagesTable . "`");
        $deleteCount = (int) mysqli_affected_rows($this->dbh);
    ...
    private function cleanupOptions()
    {
        ...
        DUPX_Log::info("CLEAN OPTIONS [" . $optionsTable . "]" . DUPX_Log::varToString($deleteOptionConds));

        DUPX_DB::mysqli_query($this->dbh, "DELETE FROM `" . $optionsTable . "` WHERE " . implode(' OR ', $deleteOptionConds));
        $deleteCount = (int) mysqli_affected_rows($this->dbh);
    ...
    {
        DUPX_Log::info("CLEAN EXTRA");
        $this->dbConnect();
        if (!$this->dbobj_views) {
            $this->dropViews();
            DUPX_Log::info("/t - VIEWS DROPPED");
        } else {
            DUPX_Log::info("/t - SKIP DROP VIEWS");
        }
        ...
        if (!$this->dbobj_procs) {
            ...
            DUPX_Log::info("/t - SKIP DROP PROCS");
        }

        if (!$this->dbobj_funcs) {
            $this->dropFuncs();
            DUPX_Log::info("/t - FUNCS DROPPED");
        } else {
            DUPX_Log::info("/t - SKIP DROP FUNCS");
        }
    }
    ...
    private function dropFuncs()
    {
        ...
        if (!($result = DUPX_DB::mysqli_query($this->dbh, $sql))) {
            ...
            DUPX_Log::info("FUNCTION CLEAN ERROR: Could not get list of FUNCTIONS to drop them.");
            return;
        }
    ...
    public function writeLog()
    {
        ...
        if ($this->dbtable_count == 0) {
            $tablePrefix = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX);
        ...
        $finalReport                              = $paramsManager->getValue(DUPX_Params_Manager::PARAM_FINAL_REPORT_DATA);
        ...
        $paramsManager->setValue(DUPX_Params_Manager::PARAM_FINAL_REPORT_DATA, $finalReport);
    ...
    {
        return $this->post['first_chunk'] || !DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_DB_CHUNK);
    }
